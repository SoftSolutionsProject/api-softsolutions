...
export class CertificadoController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Certificado gerado em PDF (arquivo)',
  })
  async emitir(
    ...
    const pdfBuffer = await this.emitirCertificadoUseCase.execute(
      idInscricao,
      idUsuario,
    );
  ...
  async validar(@Param('numeroSerie') numeroSerie: string) {
    const certificado =
      await this.certificadoRepo.findByNumeroSerie(numeroSerie);
