...
export class EmitirCertificadoUseCase {
  ...
  async execute(idInscricao: number, idUsuario: number): Promise<Buffer> {
    const inscricao: InscricaoModel | null =
      await this.inscricaoRepo.findById(idInscricao);
  ...
  private async gerarPdf(
    ...
    doc
      .strokeColor('#3498DB')
      .lineWidth(2)
      .moveTo(doc.page.width * 0.25, doc.y)
      .lineTo(doc.page.width * 0.75, doc.y)
      .stroke();
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#2980B9')
      .text(usuario.nomeUsuario.toUpperCase(), { align: 'center' });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#27AE60')
      .text(`"${curso.nomeCurso.toUpperCase()}"`, { align: 'center' });
    ...
    doc
      ...
      .fillColor('#7F8C8D')
      .text(`com carga horária de ${curso.tempoCurso || 'XX'} horas`, {
        align: 'center',
      });
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor('#7F8C8D')
      .text(`Data de emissão: ${this.formatarData(dataEmissao)}`, 50, yRodape, {
        align: 'left',
      })
      .text(`Nº do certificado: ${numeroSerie}`, -50, yRodape, {
        align: 'right',
      });
